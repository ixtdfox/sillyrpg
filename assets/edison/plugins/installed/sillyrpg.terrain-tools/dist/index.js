...
class TerrainToolsPluginRuntime {
  constructor() {
    ...
    this.persistedDraftSettings = null;
  ...
  loadPersistentState() {
    const state = this.readPersistentState();
    ...
    if (typeof state.activeBrushTool === "string") {
      this.activeBrushTool = state.activeBrushTool;
    }
    if (TERRAIN_TEXTURE_LAYERS.some((layer) => layer.id === state.activeTextureLayerId)) {
      this.activeTextureLayerId = state.activeTextureLayerId;
    }
    if (isPlainObject(state.brush)) {
      this.brush = {
        shape: state.brush.shape === "square" ? "square" : "circle",
        radius: clampFinite(state.brush.radius, 0.5, 100, this.brush.radius),
        strength: clampFinite(state.brush.strength, 0.1, 100, this.brush.strength),
        falloff: clampFinite(state.brush.falloff, 0, 1, this.brush.falloff),
        targetHeight: clampFinite(state.brush.targetHeight, -1000, 1000, this.brush.targetHeight),
        snapHeightStep: clampFinite(state.brush.snapHeightStep, 0.25, 16, this.brush.snapHeightStep)
      };
    }
    if (isPlainObject(state.draft)) {
      this.persistedDraftSettings = clone(state.draft);
    }
  }
  ...
  savePersistentState() {
    ...
    const state = {
      ...
      activeColorPreset: this.activeColorPreset,
      activeBrushTool: this.activeBrushTool,
      activeTextureLayerId: this.activeTextureLayerId,
      brush: clone(this.brush),
      draft: this.createPersistentDraftSettings()
    };

    this.context.preferences?.setPluginValue?.(manifest.id, PREFERENCES_KEY, state);
    if (!this.context.preferences?.setPluginValue) {
      try {
        window.localStorage.setItem(`edison.plugin.${manifest.id}.${PREFERENCES_KEY}`, JSON.stringify(state));
      } catch {
  ...
  readPersistentState() {
    const fallback = null;
    const stored = this.context?.preferences?.getPluginValue?.(manifest.id, PREFERENCES_KEY, fallback);
    if (stored) {
      return stored;
    }

    try {
      const raw = window.localStorage.getItem(`edison.plugin.${manifest.id}.${PREFERENCES_KEY}`);
  ...
  createPersistentDraftSettings() {
    if (!this.draft) {
      return this.persistedDraftSettings;
    }

    return {
      size: clone(this.draft.size),
      terrainGridStep: this.draft.terrainGridStep,
      resolutionMode: this.draft.resolutionMode,
      generator: clone(this.draft.generator),
      material: clone(this.draft.material)
    };
  }

  applyPersistentDraftSettings(descriptor) {
    if (!isPlainObject(this.persistedDraftSettings)) {
      return descriptor;
    }

    const settings = this.persistedDraftSettings;
    return normalizeDescriptor({
      ...descriptor,
      size: Array.isArray(settings.size) ? settings.size : descriptor.size,
      terrainGridStep: Number.isFinite(settings.terrainGridStep)
        ? settings.terrainGridStep
        : descriptor.terrainGridStep,
      resolutionMode: settings.resolutionMode === "manual" ? "manual" : "gridStep",
      generator: isPlainObject(settings.generator)
        ? { ...descriptor.generator, ...clone(settings.generator) }
        : descriptor.generator,
      material: isPlainObject(settings.material) ? clone(settings.material) : descriptor.material
    });
  }
  ...
  syncDraftFromDocument(force = false) {
    ...
    if (terrain?.kind === "generated") {
      const appliedDraft = normalizeDescriptor(clone(terrain));
      this.draft = this.applyPersistentDraftSettings(appliedDraft);
      this.appliedTerrainSignature = nextSignature;
      this.draftDirty = terrainSignature(this.draft) !== terrainSignature(appliedDraft);
    ...
    this.textureMapLoadToken = null;
    this.textureSaveBakeRequired = false;
    this.draft = this.applyPersistentDraftSettings(normalizeDescriptor(createDescriptorFromPreset(DEFAULT_PRESET, {
    ...
    this.appliedTerrainSignature = nextSignature;
    this.draftDirty = isPlainObject(this.persistedDraftSettings);
  }
  ...
  selectPreset(presetId, host) {
    ...
    this.persistedDraftSettings = this.createPersistentDraftSettings();
    this.savePersistentState();
    this.render(host);
  }
